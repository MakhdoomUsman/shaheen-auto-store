...
const TotalTable = ({ invoiceDetails }) => {
  ...
  useEffect(() => {
    invoiceDetails?.details?.map((data, index) => (
      setVatTax(data?.tax_applied),
      setSubtotalDiscount(data?.discount)
    ))
  }, []);
      ...
      <div className="md:flex px-6 py-6 items-center">
        ...
        <div className="flex-none min-w-[270px] space-y-3">
          <div className="flex justify-between">
            ...
            <span className="text-slate-900 dark:text-slate-300">USD {invoiceDetails?.subtotal}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-medium text-slate-600 text-xs dark:text-slate-300 uppercase">
              Discount Total:
            </span>
            <span className="text-slate-900 dark:text-slate-300">{subtotalDiscount} %</span>
          </div>
          <div className="flex justify-between">
            <span className="font-medium text-slate-600 text-xs dark:text-slate-300 uppercase">
              Vat:
            </span>
            <span className="text-slate-900 dark:text-slate-300">{vatTax} %</span>
          </div>
          <div className="flex justify-between">
            <span className="font-medium text-slate-600 text-xs dark:text-slate-300 uppercase">
              Invoice total:
